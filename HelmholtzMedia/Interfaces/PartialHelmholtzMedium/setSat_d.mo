...
algorithm
  ...
  elseif (d>=dl_trip) or (d<=dv_trip) then
    ...
    sat.Tsat:= T_trip;
    sat.psat:= p_trip;
    sat.liq := setState_dTX1p(d=dl_trip, T=T_trip);
    sat.vap := setState_dTX1p(d=dv_trip, T=T_trip);
  else
    // Modelica.Utilities.Streams.print("d=d_crit: return critical values", "printlog.txt");
    sat.Tsat := T_crit;
    sat.psat := p_crit;
    sat.liq := setState_dTX1p(d=d_crit, T=T_crit);
    sat.vap := setState_dTX1p(d=d_crit, T=T_crit);
  end if;
